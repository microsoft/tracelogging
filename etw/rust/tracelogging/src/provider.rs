...
/// 
/// Use the `define_provider` macro to create a provider.
/// 
/// Use the `write_event` macro to write events.
/// 
/// ```
...
impl Provider {
    ...
    ///
    /// The thread-local activity id will be used if write is called with an activity_id of None.
    ...
    /// Gets the provider metadata bytes.
    ...
    /// ## Safety
    ///
    /// - If creating a DLL, all registered providers **must** be unregistered before the
    ///   DLL unloads. You'll typically register the provider for `DLL_PROCESS_ATTACH`
    ///   and unregister for `DLL_PROCESS_DETACH`. The provider cannot unregister itself
    ///   because the provider is static and Rust does not drop static objects. If a
    ///   provider variable is registered by a DLL and the DLL unloads while the provider
    ///   is still registered, the process may subsequently crash. This occurs because
    ///   `register` enables an ETW callback into the calling DLL and `unregister`
    ///   ensures that the callback is disabled. If the module unloads without disabling
    ///   the callback, the process will crash the next time that ETW tries to invoke the
    ///   callback.
