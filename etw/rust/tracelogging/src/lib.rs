...
/// not a variable. You must use the original symbol name (not a reference) when you
/// invoke [`write_event!`]. Using a reference instead of the original symbol will lead
/// to compile errors like "cannot find value \`_TLG_PROV_MY_PROVIDER\` in this scope".
///
...
///
///   This must be the original symbol name created by [`define_provider!`]. Using a
///   reference instead of the original symbol will lead to compile errors like
///   "cannot find value \`_TLG_PROV_MY_PROVIDER\` in this scope".
///
/// - `"EventName"`
///
